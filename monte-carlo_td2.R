...
print(count-1)#count-1 suit une loi géométrique.
...
t0=proc.time()
count<-0
while(count<100000){ #On va simule 100000 observations qui suit une loi normale.


ech_ber<-(ceiling((runif(1)-1/2))-1/2)*2# simuler une loi de bernoulli à valeur à {+1,-1}
ech<--log(runif(1))*ech_ber# simuler une loi de Laplace
...
#4.1
f<- function (x) (1/(6*sqrt(2*pi)))*exp(-(x+3)^2/2)+(2/(3*sqrt(2*pi))*exp(-(x-3)^2/2))
plot(f,xlim = c(-10,10),main="Représentation de f")

#4.2&4.3
#on regard X comme une loi mélange de N(-3,4  ) et N(3,1)
...
#il ne coincide pas parfaitement !
...
u<-pnorm(X[,1])
v<-pnorm(X[,2])
#(u,v)est une échantiollon dont la fonction de repartion égale à la copule gaussien de 
#paramètre p.
